...
export const Input=< T extends FieldValues >  ({
    ...
    return (
        <div className="w-full">
            {
                label && (
                    <label className={`block  text-gray-700 font-semibold mb-2 ${errors && errors?.isValid && 'text-red-500'}`} htmlFor={String(name)}>
                        {label}
                    </label>
                )
            }
            <input 
                type={type}
                className={`${variant} ${ disabled && 'cursor-not-allowed bg-gray-200 opacity-40'} ${errors && errors?.isValid && '!border-red-500'} w-full ${ className }`}
